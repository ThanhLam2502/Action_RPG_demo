# Task List

| Task ID | Nội dung                                   | Kết quả                          | Status | Depends On |
|:--------|:-------------------------------------------|:---------------------------------|:-------|:-----------|
| T001    | Khởi tạo repository                        | Có Github repository             | [x]    | -          |
| T002    | Setup Repository structure                 | Folder Action_RPG_demo           | [x]    | -          |
| T003    | Định hướng tầm nhìn của dự án              | docs/01_project.md               | [x]    | -          |
| T004    | Xác định việc cần làm trong từng giai đoạn | docs/04_development_roadmap      | [x]    | -          |
| ---     | ---                                        | ---                              | ---    | ---        |
| T005    | Tạo cube Hero                              | Hero tượng trưng bằng cube       | [x]    | -          |
| T006    | Charactor movement                         | nhân vật di chuyển bằng W,A,S,D  | [x]    | -          |
| T007    | Charactor normal attack (mele)             | Hero đánh thường bằng chuột trái | [x]    | -          |
| T008    | Dash                                       | Có thể dash                      | [ ]    | -          |
| T00-    | -                                          | -                                | [ ]    | -          |
| T00-    | Gather                                     | -                                | [x]    | -          |

